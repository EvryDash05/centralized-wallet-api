...
export async function findParticipantByAppName(appName) {
    const response = await DatabaseConnection.query(PARTICIPANTS_QUERIES.FIND_PARTICIPANT_BY_APP_NAME, [appName]);
    return response.rows[0];
}